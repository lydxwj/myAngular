...
$(function () {
    /*===================菜单 JS 动作=====================*/
    var menus = $(".nav-con ul>li:first").siblings().andSelf();
    ...
    //给定 thisli 默认位置
    thisMenu.css({ left: (mNow * 111 + mt) + "px" });
    menus.hover(function () {
        MenuMove($(this), ($(this).index() * 111 + mt), 1)
    }, function () {
        MenuMove($(this), (mNow * 111 + mt), 0)
    });
    var MenuAuto;
    function MenuMove(tn, lefts, move) {
        ...
        var _h = nodes.children("ul").height() + 15;//15个像素是 div 的 padding 值
        if (move === 1) {
            thisMenu.stop();
