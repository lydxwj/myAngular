...
$(function() {
	//鼠标点击图片切换
	$('.slider1 ol li').click(function(e) {
		var index = $(this).index();
		effect(index);
		num = index;
		$('.slider1 ol img').removeClass('on').eq(num).addClass('on');
	});
	...
	var timer = null;
	var speed = 3000;
	var liLength = $('.slider1 ul li').length; //获取li长度
	var myFn = function() {
		num++;
		if (num > (liLength - 1)) {
			num = 0;
